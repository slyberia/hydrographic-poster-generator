...
test("Drone homepage presents the merged overview with an interactive map", async ({ page }) => {
  ...
  await expect(page.locator(".cap-screen")).toHaveCount(4);
  await expect(page.locator(".cap-screen--scenario")).toHaveCount(1);
  ...
  await expect(page.getByText("PDF and GeoJSON exports are planned", { exact: false })).toHaveCount(0);
});
