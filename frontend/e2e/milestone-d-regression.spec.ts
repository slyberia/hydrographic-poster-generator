...
test("route ownership and the two-view entry flow remain explicit", async ({ page, request }) => {
  ...
  expect(legacy.headers().location).toBe("/workspace/drone");
  ...
  expect(legacyDocumentation.headers().location).toBe("/workspace/drone/docs/executive-overview");
...
test("platform product cards are complete keyboard-focusable links", async ({ page }) => {
  ...
  expect(hrefs).toEqual(["/poster", "/documentation"]);
...
test("platform hero presents the poster artifact without loading drone zoning", async ({ page }) => {
  const requests = await mockPublishedMap(page, { delayMs: 750 });
  await page.goto("/");

  await expect(page.getByRole("img", { name: "Guyana hydrographic poster preview" })).toBeVisible();
  expect(requests.some((url) => url.includes("/workspace/drone/zoning"))).toBe(false);
